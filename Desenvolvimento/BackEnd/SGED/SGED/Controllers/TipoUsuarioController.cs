...
public class TipoUsuarioController : Controller
{
	...
	public async Task<ActionResult<IEnumerable<TipoUsuarioDTO>>> Get()
	{
		...
		if (tipoUsuarioDTO == null) return NotFound("Tipo usuário not found!");
	...
	{
		if (tipoUsuarioDTO is null) return BadRequest("Data Invalid!");
	...
	{
		var tipoUsuarioDTO = await _service.GetById(id);
		if (tipoUsuarioDTO == null) return NotFound("TipoUsuario not found!");
